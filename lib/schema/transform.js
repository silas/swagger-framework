...
function parameters(spec, paramType) {
  ...
  spec.parameters.forEach(function(parameter) {
    ...
    case 'array':
      ...
      if (parameter.hasOwnProperty('minItems')) {
        property.minItems = parameter.minItems;
      }

      if (parameter.hasOwnProperty('maxItems')) {
        property.maxItems = parameter.maxItems;
      }
    ...
    if (parameter.allowMultiple &&
      ...
      var schemaProperty = schema.properties[parameter.name] = {
        type: 'array',
        items: property,
      };

      if (parameter.uniqueItems) schemaProperty.uniqueItems = true;

      if (parameter.hasOwnProperty('minItems')) {
        schemaProperty.minItems = parameter.minItems;
      }

      if (parameter.hasOwnProperty('maxItems')) {
        schemaProperty.maxItems = parameter.maxItems;
      }
    }
  });
