...
var verboseDebug = require('debug')('swagger-framework:api');
...
function Api(spec) {
  ...
  debug('create api ' + spec.path);
  verboseDebug('create api', spec);
...
Api.prototype.setup = function(framework) {
  ...
  debug('setup api ' + this.spec.path);
...
Api.prototype.resource = function(spec) {
  ...
  debug('register resource ' + spec.path);
