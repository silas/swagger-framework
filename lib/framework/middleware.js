...
function header(operation) {
  ...
  var produces = operation.spec.produces ||
...
function consumes(operation) {
  ...
  var mimes = operation.spec.consumes || operation.resource.api.consumes;

  return function(req, res, next) {
    if (mimes && !is(req, mimes)) {
...
function bodyForm(operation) {
  if (['HEAD', 'GET'].indexOf(operation.spec.method) >= 0) return;

  var env = operation.resource.api.framework.env;
  var schema = utils.createParamTypeSchema('form', operation.spec);
  var types = ['application/x-www-form-urlencoded'];
...
exports.bodyRaw = bodyRaw;
exports.bodyForm = bodyForm;
